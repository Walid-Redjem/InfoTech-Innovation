...
export default function MobileBottomNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 inset-x-0 z-40 bg-white/90 dark:bg-gray-900/90 backdrop-blur-md border-t border-lilac-dark/30 dark:border-gray-700 pb-safe">
        ...
        {navItems.map(({ href, icon: Icon, key }) => {
          ...
          return (
            <Link key={href} href={full} className="flex flex-col items-center gap-0.5 min-w-[52px]">
              <div className={`relative p-1.5 rounded-xl transition-all ${active ? "bg-mauve text-white shadow-md shadow-mauve/30" : "text-gray-400 dark:text-gray-500"}`}>
              ...
              <span className={`text-[10px] font-medium transition-colors ${active ? "text-mauve" : "text-gray-400 dark:text-gray-500"}`}>
                {t(key as "home")}
              </span>
            </Link>
          );
